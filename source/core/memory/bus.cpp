	...
	dma(this), cddrive(this)
{
	...
	gl_renderer->init_debugger(this);
...
{
	if (addr % sizeof(T) != 0) {
		printf("[BUS] Bus::read: unaligned read at address: 0x%x\n", addr);
		exit(1);
	}
	...
	int width = sizeof(T);
	printf("[BUS] Bus::read: unhandled read to address: 0x%x with width %d\n", abs_addr, width);
 	exit(1);
	return 0;
}
...
{
	if (addr % sizeof(T) != 0) {
		printf("[BUS] Bus::write: unaligned write to address: 0x%x\n", addr);
		exit(1);
	}
	...
	int width = sizeof(T);
	printf("[BUS] Bus::write: unhandled write to address: 0x%x with width %d\n", abs_addr, width);
	exit(1);
}
